...
import { kvGet, kvGetCount, kvSet, tạoKeyKV } from "./Hàm cho KV.ts";
...
export async function xácĐịnhIdTrênLocal(
  ...
  const tổngSốEntryHiệnTại = await kvGetCount(viếtHoa(tênDanhSách) as TênBảng);
...
export interface SốLượngDữLiệu {
  "Bài đăng": SốLượngBàiĐăng;
  "Nơi đăng": SốLượngNơiĐăng;
  "Đuôi rút gọn": number;
}
...
export async function cậpNhậtSốLượngBàiĐăng(value: SốLượngBàiĐăng | PhươngThứcTạoBàiĐăng, delta: number = 1) {
  ...
  if (typeof value === "object") {
    await kvSet(key, value, "cậpNhậtSốLượng");
